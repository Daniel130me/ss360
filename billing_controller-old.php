...
switch ($action) {
    ...
    case 'get_student_payment_record':
        ...
        // Validate input
        if (!$term_id || !$session_id || !$class_id) {
    ...
    case 'update_bill':
        try {
            ...
            // If amount_paid > 0, update last_payment to NOW(), else do not update last_payment
            // if ($amount_paid > 0) {
    ...
    case 'record_payment':
        try {
            ...
            $date_paid = filter_input(INPUT_POST, 'payment_date', FILTER_SANITIZE_STRING);
            $createdby = $user_id;
            $datecreated = $date;
            ...
            // exit;
            // Insert payment log
            $stmt = $conn->prepare("INSERT INTO payment_log (total_amount_paid, description, amount_newly_paid, balance, payment_method_id, student_id, school_id, session_id, term_id, class_id, status, date_paid, createdby, datecreated, bill_id) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param(
                "dsddiiisssisisi",
                $total_amount_paid,
            ...
            function get_stats($conn, $school_id, $session_id, $term_id, $class_id, $bill_type)
            {
                ...
                if (count($all_bill_ids) === 0) {
                    return [
                        ...
                        'students_paid' => 0,
                        'students_completed' => 0,
                        'students_unpaid' => $total_students,
                        'total_due' => $total_due,
                        'total_paid' => 0,
