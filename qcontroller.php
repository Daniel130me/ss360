...
?>
    ...
    if ($action === 'get_subject_by_category') {
        echo $school_id = $_SESSION['school_id'];
        echo $filtertype = $_POST['datatype'];
        // $data = [];
        // if ($filtertype == 'All') {
        //     $query = "SELECT id,subject FROM subjects ORDER BY subject ASC";
        //     $select = mysqli_query($conn, $query);
        ...
        echo $query = "SELECT id,subject_ids FROM subject_cat WHERE $filter AND school_id='$school_id'";
        $select = mysqli_query($conn, $query);
        // }
    ?>
        <?php
