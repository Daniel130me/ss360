...
<?php
    ...
    if ($action == 'delete_int_msg') {
        ...
        mysqli_stmt_close($stmt);
        if ($ok) echo json_encode(['status' => '1']);
        else echo json_encode(['status' => '0', 'err' => 'Delete failed']);
        exit;
    }
    ...
    if ($action == 'update_internal_msg') {
        ...
        // echo $userid;
        if ($owner != $userid) {
            echo json_encode(['status' => '0', 'err' => 'Permission denied']);
            exit;
        }
        ...
        mysqli_stmt_close($stmt);
        if ($ok) echo json_encode(['status' => '1']);
        else echo json_encode(['status' => '0', 'err' => 'Update failed']);
        exit;
    }
