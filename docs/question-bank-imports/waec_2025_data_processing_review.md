# WAEC 2025 Data Processing Import Review

Source: myschool.ng classroom listing `https://myschool.ng/classroom/data-processing?exam_type=waec&exam_year=2025`
(collected via the public `api/web/v1/classroom/data-processing` JSON, all 9 listing
...
- Question category: Data Processing (generic fallback; no per-item topic invented)
- Difficulty: Medium (default; not rated from source)
- Review status: approved (matches existing exam-body imports)

## Extraction Summary

- Listing pages visited: 9 (5 questions each, 44 total)
- Individual question pages opened: 44 (plus 0 failed fetches, if any)
...
- Images: 0 question diagram(s) downloaded from `myschool.ng/storage/classroom/`,
  ...
  `uploads/question_bank/waec_2025_data_processing/`; referenced with the established
  `../uploads/...` relative path.
- Answer images (`answer_image` from `myschool.ng/storage/classroom_answers/`):
  1 embedded after manual live-page review confirmed the stem requires
  the image.
- Answer source: the `correct_option` object on each individual question page
  ...
  agreed with individual-page flags for all 40 imported questions.
...
- `question_category` uses the generic subject fallback 'Data Processing' because
  the source provides no per-question topic; assigning syllabus topics
  would have required inventing classifications.
- Questions: 40; options: 160; exactly one `answer = 1` per question.
...
- Item 41: 0 options (theory), option tags not a-d: [], correct flag != 1, no correct_option on individual page, type=theory (https://myschool.ng/classroom/data-processing/77464?exam_type=waec&exam_year=2025)
- Item 42: 0 options (theory), option tags not a-d: [], correct flag != 1, no correct_option on individual page, type=theory (https://myschool.ng/classroom/data-processing/77465?exam_type=waec&exam_year=2025)
- Item 43: 0 options (theory), option tags not a-d: [], correct flag != 1, no correct_option on individual page, type=theory (https://myschool.ng/classroom/data-processing/77466?exam_type=waec&exam_year=2025)
- Item 44: 0 options (theory), option tags not a-d: [], correct flag != 1, no correct_option on individual page, type=theory (https://myschool.ng/classroom/data-processing/77469?exam_type=waec&exam_year=2025)
