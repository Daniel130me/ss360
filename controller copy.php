...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'upload_lesson_image') {
        ...
        if (!$fileKey || !isset($_FILES[$fileKey]) || !is_uploaded_file($_FILES[$fileKey]['tmp_name'])) {
            echo json_encode(['success' => false, 'err' => 'No file uploaded']);
            exit;
        }
        ...
        if (!in_array($ext, $allowed)) {
            echo json_encode(['success' => false, 'err' => 'Invalid file type']);
            exit;
        }

        if ($file['size'] > 2 * 1024 * 1024) { // limit 2MB
            echo json_encode(['success' => false, 'err' => 'File too large (max 2MB)']);
            exit;
        }
        ...
        if (move_uploaded_file($file['tmp_name'], $dest)) {
            ...
            echo json_encode(['success' => true, 'url' => $url, 'name' => $safeName]);
            exit;
        } else {
            echo json_encode(['success' => false, 'err' => 'Failed to move uploaded file']);
            exit;
        }
    }
    if ($action == 'settings') {
...
?>
    ...
    ?>
        ...
        if ($action == 'update_class_data') {
            ...
            // echo "UPDATE class SET $set classname='$classname', dateupdated='$date', updatedby='{$_SESSION['userid']}' WHERE school_id='$school_id' AND id='$id'";
            // exit;
