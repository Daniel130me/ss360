...
$(document).ready(function () {
            ...
            success: function (res) {
                ...
                const invoiceHtml = `
                    <div class="card">
                        <div class="card-body p-3" id="printableInvoice">
                            <div class="d-flex align-items-center mb-3">
                                ...
                                <div class="flex-fill">
                                    ...
                                    <small class="text-muted">Invoice Preview</small>
                                </div>
                                <div class="text-right">
                                    <div><strong>${escapeHtml(student.firstname || '')} ${escapeHtml(student.lastname || '')}</strong></div>
                                    <small class="text-muted">Admission: ${escapeHtml(student.admission_no || '')}</small>
                                </div>
                            </div>
                        ...
                        <div class="card-footer bg-white text-right"><small class="text-muted">Bill ID: ${escapeHtml(String(bill.id || ''))} • Date: ${escapeHtml(String(bill.datecreated || ''))}</small></div>
                    </div>
                `;
    ...
    // Print and download handlers for invoice modal
                            ...
                            return `
                                ...
                                <button class="btn btn-xs btn-primary edit-bill" data-id="${row.id}" data-student_id="${row.student_id}">Update Assigned Bill</button>
                                    <button class="btn btn-xs btn-primary record-payment" data-amountdue="${row.amount_due}" data-balance="${row.balance}" data-id="${row.id}" data-student_id="${row.student_id}">Record Payment</button>
                                        ...
                                        <button class="btn btn-xs btn-danger delete-bill" data-bill_id="${row.id}" data-student_id="${row.student_id}" data-class_id="${row.class_id}" data-session_id="${row.session_id}" data-term_id="${row.term_id}">Delete Assigned Bill</button>
                                </div>
                            `;
                        } else {
                            return `
    ...
    $(document).on('click', '.delete-bill', function () {
        ...
        // Populate modal with info and show
        $('#deleteAssignedBillModal').data('bill-id', billId);
        $('#deleteAssignedBillModal').data('student-id', studentId);
        $('#deleteAssignedBillModal').data('class-id', classId);
        $('#deleteAssignedBillModal').data('session-id', sessionId);
        $('#deleteAssignedBillModal').data('term-id', termId);
        $('#deleteAssignedBillModal').modal('show');
    });

    // Confirm deletion button in modal
    $(document).on('click', '#confirmDeleteAssignedBill', function () {
        const $modal = $('#deleteAssignedBillModal');
        const billId = $modal.data('bill-id');
        const studentId = $modal.data('student-id');
        const classId = $modal.data('class-id');
        const sessionId = $modal.data('session-id');
        const termId = $modal.data('term-id');
        ...
        $.ajax({
            ...
            success: function (res) {
                $('#confirmDeleteAssignedBill').prop('disabled', false).text('Delete');
                if (res && res.success) {
                    $('#deleteAssignedBillModal').modal('hide');
                    toastr.success(res.message || 'Bill deleted');
                    if ($.fn.DataTable && $('#student_payment_recordTable').length) {
                        $('#student_payment_recordTable').DataTable().ajax.reload();
                    }
                } else {
                    toastr.error(res.message || 'Failed to delete bill');
                }
            },
            error: function (xhr, status, err) {
                $('#confirmDeleteAssignedBill').prop('disabled', false).text('Delete');
                toastr.error('Server error while deleting bill');
            }
        });
    });
