...
function ensureMathQuillPluginRegistered() {
        ...
        self.initialize = function () {
          ...
          $("#mathQuillModal").on("shown.bs.modal", function () {
            ...
            loadMathQuillResources(function () {
              ...
              if (mathFieldInstance) {
                try {
                  mathFieldInstance.revert();
                } catch (e) {}
                mathFieldInstance = null;
              }
              $inputField.empty();
              if (_mathquill_loader.MQ) {
                mathFieldInstance = _mathquill_loader.MQ.MathField(
                  $inputField[0],
                  {
                    spaceBehavesLikeTab: true,
                    handlers: {
                      edit: function () {
                        try {
                          $("#latex-output").text(mathFieldInstance.latex());
                        } catch (e) {}
                      },
                    },
                  },
                ...
                // expose the current field globally for the insert handler
                try {
                  window.__summernoteMathCurrentField = mathFieldInstance;
                } catch (e) {}
          ...
          $("#mathQuillModal").on("hidden.bs.modal", function () {
            ...
            // Clear current math field reference
            try {
              window.__summernoteMathCurrentField = null;
            } catch (e) {}
          });
          ...
          if (!window.__summernoteMathInsertHandlerRegistered) {
            ...
            $(document).on("click", "#insert-equation-btn", function () {
              // The mathFieldInstance used inside shown.bs.modal is stored on window for cross-scope access
              var mathField =
                window.__summernoteMathCurrentField ||
                mathFieldInstance ||
                null;
              if (!mathField) {
                $("#mathQuillModal").modal("hide");
              ...
              if ($target && $target.length) {
                ...
                try {
                  var nodeHtml =
                    '<span contenteditable="false" class="math-editor-rendered" data-latex="' +
                    latex.replace(/"/g, "&quot;") +
                    '"></span>';
                  // insert directly into the editable DOM
                  $(editableElem).append(nodeHtml);
              ...
              if (!($target && $target.length) && !editableElem) {
                ...
                try {
                  if (
                    window.__summernoteMathEditorContext &&
                    typeof window.__summernoteMathEditorContext.invoke ===
                      "function"
                  ) {
                    var nodeHtml =
                      '<span contenteditable="false" class="math-editor-rendered" data-latex="' +
                      latex.replace(/"/g, "&quot;") +
                      '"></span>';
                    window.__summernoteMathEditorContext.invoke(
                      "editor.insertNode",
                      $(nodeHtml)[0],
                    );
                    try {
                      var $editable = $(
                        window.__summernoteMathEditorContext.layoutInfo
                          .editable,
                      );
                      renderEquationsIn($editable);
                    } catch (e) {}
                ...
                if (!inserted && window.__summernoteMathEditorElement) {
                  try {
                    var nodeHtml =
                      ...
                      latex.replace(/"/g, "&quot;") +
                      '"></span>';
                    window.__summernoteMathEditorElement.summernote(
                      "insertNode",
                      $(nodeHtml)[0],
                    );
                    try {
                      var $editable = window.__summernoteMathEditorElement
                        .closest(".note-editor")
                        .find(".note-editable");
                      renderEquationsIn($editable);
                    } catch (e) {}
          ...
          var button = ui.button({
            ...
            click: function () {
              ...
              try {
                ...
                var $editable = $(context.layoutInfo.editable);
                // store the editable element explicitly so insertions target the correct editor
                window.__summernoteMathEditorEditable =
                  context.layoutInfo.editable;
                var $textarea = $editable
                  .closest(".note-editor")
                  .prev("textarea");
                if ($textarea && $textarea.length)
                  window.__summernoteMathEditorElement = $textarea;
              } catch (e) {
                window.__summernoteMathEditorContext = null;
                window.__summernoteMathEditorElement = null;
              }
  ...
  if (!registerPlugin()) {
    ...
    var retry = function () {
      attempts++;
      if (!registerPlugin() && attempts < 20) setTimeout(retry, 200);
    };
...
function displayQuestion(questionData) {
  ...
  try {
    var hasMathSpan =
      ...
      container.querySelector(
        "span.math-editor-rendered, .mq-root, .mq-selectable",
      );
    if (!hasMathSpan) {
  ...
  try {
    $(container)
      .find(".options .icheck-gray-dark, .option-group")
      .each(function () {
        var $this = $(this);
        ...
        if (hasMath && text.length === 0) {
          ...
          $this
            .find("span.math-editor-rendered")
...
function toggle_assessment_btn(btn) {
...
function updateNavigationPanel() {
  ...
  for (let i = 1; i <= totalQuestions; i++) {
    ...
    const btn = $(
      `<button class="btn ${btnClass}" onclick="loadQuestion(${i})">${i}</button>`,
    );
...
function setupAnswerHandling() {
  ...
  $(document).on("change", 'input[name="answer"]', function () {
    ...
    const questionId = $(this)
      ...
      .data("question-id"); // Get the real question ID
...
// Import Questions Feature - Global Variables
var importQuestionsState = {
...
// Open import modal and load initial data
...
// Load subjects into the filter dropdown
function loadSubjectsForImport() {
  $.ajax({
    url: "../controller_new.php",
    type: "POST",
    data: {
      action: "get_all_subjects_teacher",
    },
    success: function (response) {
      try {
        const data =
        ...
        $("#import_subject_filter").html(options);
      } catch (e) {
...
// Load filtered questions based on selected Term, Type, and Subject
function loadFilteredQuestions(page = 1) {
  const term = $("#import_term_filter").val();
  const assessmentType = $("#import_type_filter").val();
  const subjectId = $("#import_subject_filter").val();

  // Validate that all filters are selected
  if (term === "all" || assessmentType === "all" || subjectId === "all") {
    $("#import-questions-list").html(
      '<p class="text-muted text-center py-5">Please select Term, Type, and Subject to load questions.</p>',
    );
    $("#import_pagination").hide();
    return;
  }

  importQuestionsState.currentPage = page;

  // Get current assessment ID to exclude it
  ...
  $.ajax({
    ...
    type: "POST",
    data: {
      action: "get_questions_for_import",
      term: term,
      type: assessmentType,
      subject_id: subjectId,
      current_assessment_id: currentAssessmentId,
      page: page,
      per_page: importQuestionsState.perPage,
    },
    beforeSend: function () {
      $("#import-questions-list").html(
      ...
      if (data.success && data.questions.length > 0) {
        ...
        data.questions.forEach((question, index) => {
          const questionNumber =
            (page - 1) * importQuestionsState.perPage + index + 1;
          const isChecked = importQuestionsState.selectedQuestions.some(
            (q) => q.id === question.id,
          );

          html += `
                        <div class="question-import-item mb-3 p-2" style="border: 1px solid #ddd; border-radius: 5px;">
                            <div class="d-flex align-items-start">
                                <input type="checkbox" class="import-question-checkbox mr-2 mt-1" 
                                       data-question-id="${question.id}" 
                                       ${isChecked ? "checked" : ""}
                                       onchange="toggleQuestionImportSelection(${question.id}, this.checked)">
                                <div class="flex-grow-1">
                                    <div class="mb-1">
                                        <span class="badge badge-info">Q${questionNumber}</span>
                                        <small class="text-muted ml-2">${question.subject || ""} | ${question.type_name || ""}</small>
                                    </div>
                                    <div class="question-text">${question.question}</div>
                                    <div class="mt-2 options-preview row">
                                        ${question.options
                                          .map(
                                            (opt, i) => `
                                            <div class="col-6 mb-1 ${opt.isAnswer ? "text-success font-weight-bold" : ""}">
                                                ${String.fromCharCode(65 + i)}. ${opt.text}
                                                ${opt.isAnswer ? ' <i class="fas fa-check-circle ml-1"></i>' : ""}
                                            </div>
                                        `,
                                          )
                                          .join("")}
        ...
        // Render MathQuill equations in the container
        const $container = $("#import-questions-list");
        if (typeof convertDollarLatexToSpans === "function") {
          $container.each(function () {
            convertDollarLatexToSpans(this);
          });
        }
        if (typeof renderEquationsIn === "function") {
          renderEquationsIn($container);
        }

        // Update pagination
        importQuestionsState.totalPages = data.pagination.total_pages;
        $("#import_current_page").text(page);
        $("#import_total_pages").text(data.pagination.total_pages);
        $("#import_prev_btn").prop("disabled", page <= 1);
        $("#import_next_btn").prop(
          "disabled",
          page >= data.pagination.total_pages,
        );
        $("#import_pagination").show();
      } else {
        $("#import-questions-list").html(
          '<p class="text-muted text-center py-5">No questions found matching your selection.</p>',
        );
        $("#import_pagination").hide();
      }
    },
    error: function () {
      ...
      $("#import-questions-list").html(
        '<p class="text-danger text-center py-5">Error loading questions. Please try again.</p>',
      );
    },
  });
}
...
function toggleQuestionImportSelection(questionId, isChecked) {
  if (isChecked) {
    // Extract question data from the rendered list item
    const questionData = extractQuestionData(questionId);
    if (questionData) {
      importQuestionsState.selectedQuestions.push(questionData);
    }
  } else {
    // Remove from selected list
    importQuestionsState.selectedQuestions =
      importQuestionsState.selectedQuestions.filter((q) => q.id !== questionId);
  }
}

// Extract question data from the rendered preview
function extractQuestionData(questionId) {
  const checkbox = $(
    `.import-question-checkbox[data-question-id="${questionId}"]`,
  );
  const questionItem = checkbox.closest(".question-import-item");

  if (questionItem.length === 0) return null;

  // Extract question HTML (raw HTML to preserve MathQuill spans)
  const questionHtml = questionItem.find(".question-text").html();

  // Extract options
  const options = [];
  questionItem.find(".options-preview > div").each(function () {
    const isAnswer = $(this).hasClass("text-success");
    // Get text content, removing the A. B. prefix and icon
    let text = $(this).clone().find("i").remove().end().text().trim();
    text = text.replace(/^[A-Z]\.\s*/, "").trim();

    // If there were math spans, try to preserve them?
    // Actually, for import, we want the raw LaTeX or sanitized MathQuill.
    // However, the textarea will use Summernote, which handles HTML.
...
// Import selected questions into the current assessment
function importSelectedQuestions() {
  if (importQuestionsState.selectedQuestions.length === 0) {
    toastr.warning("Please select at least one question to import");
    return;
  }

  // Add each selected question to the questions container
  importQuestionsState.selectedQuestions.forEach((questionData) => {
    addImportedQuestion(questionData);
  });

  toastr.success(
    `Imported ${importQuestionsState.selectedQuestions.length} question(s)`,
  );
...
// Add an imported question to the questions container
function addImportedQuestion(questionData) {
  const questionCount = $("#questions-container .question-block").length + 1;

  const questionHTML = `
        <div class="question-block mb-4 p-3" style="border: 1px solid #ddd; border-radius: 5px;">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control question-textarea">${questionData.question}</textarea>
            </div>
                  ...
                  .map(
                    ...
                    <div class="option-group mb-2">
                        ...
                        <div class="input-group">
                            <textarea class="form-control option-textarea">${option.text}</textarea>
  ...
  // Reinitialize Summernote for the new textareas
  initializeSummernote();
}

// Event handlers for filter changes
$("#import_term_filter, #import_type_filter, #import_subject_filter").on(
  "change",
  function () {
    loadFilteredQuestions();
  },
...
$("#import_prev_btn").on("click", function () {
  if (importQuestionsState.currentPage > 1) {
    loadFilteredQuestions(importQuestionsState.currentPage - 1);
  }
});

$("#import_next_btn").on("click", function () {
  if (importQuestionsState.currentPage < importQuestionsState.totalPages) {
    loadFilteredQuestions(importQuestionsState.currentPage + 1);
  }
});
...
$("#import-question-btn").on("click", function () {
