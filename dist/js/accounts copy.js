...
function recalculate_balance() {
        ...
        const amountDueText = $('#amount_due_record_payment').text();
        const amountDue = parseFloat(amountDueText.replace(/[^\d.-]/g, '')) || 0;
        const amountPaid = parseFloat($('#payment_amount').val()) || 0;
        let newBalance = amountDue - amountPaid;
...
$(document).ready(function () {
    ...
    function renderQuickAssignPreview() {
        ...
        // Build branded preview card (compact for quick assign)
        let previewHtml = '';
        previewHtml += `<div class="card">`;
        previewHtml += `<div class="card-header bg-white border-0 d-flex align-items-center">`;
        previewHtml += `<div><h6 class="mb-0">Quick Assign — ${escapeHtml(billTypeName)}</h6><small class="text-muted">${escapeHtml(studentName)}</small></div>`;
        previewHtml += `<div class="ml-auto text-right"><span class="badge badge-pill badge-info">Single student</span></div>`;
        previewHtml += `</div>`;
        previewHtml += `<div class="card-body p-2">`;
        previewHtml += `<div class="row"><div class="col-md-7"><h6 class="text-secondary">Breakdown</h6><div class="table-responsive"><table class="table table-sm table-borderless mb-0"><thead><tr class="text-muted"><th>Description</th><th class="text-right">Amount</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
        previewHtml += `<div class="col-md-5"><div class="card bg-light"><div class="card-body p-2">`;
        previewHtml += `<div class="d-flex justify-content-between"><small class="text-muted">Subtotal</small><strong>₦${Number(subtotal || 0).toLocaleString()}</strong></div>`;
        previewHtml += `<div class="d-flex justify-content-between"><small class="text-muted">Deduction</small><strong>${escapeHtml(deductionPercentage || '0')}%</strong></div>`;
        previewHtml += `<div class="d-flex justify-content-between"><small class="text-muted">Tax</small><strong>${escapeHtml(tax || '0')}%</strong></div>`;
        previewHtml += `<hr class="my-2">`;
        previewHtml += `<div class="d-flex justify-content-between"><small class="text-muted">Total</small><strong>₦${Number(totalAmount || 0).toLocaleString()}</strong></div>`;
        previewHtml += `</div></div><div class="mt-2 small text-muted">Notes:</div><div class="small text-dark">${escapeHtml(notes || '') || '<span class="text-muted">No notes</span>'}</div></div></div>`;
        previewHtml += `</div><div class="card-footer text-right bg-white"><small class="text-muted">Preview — verify before assigning</small></div></div>`;
                    ...
                    render: function (data, type, row) {
                        ...
                        if (row.bill_type) {
                            return `
                                ...
                                <button class="btn btn-xs btn-primary view-bill" data-id="${row.id}">View Payment Record</button>
                                <button class="btn btn-xs btn-primary edit-bill" data-id="${row.id}" data-student_id="${row.student_id}">Update Assigned Bill</button>
                                    <button class="btn btn-xs btn-primary record-payment" data-amountdue="${row.amount_due}" data-balance="${row.balance}" data-id="${row.id}" data-student_id="${row.student_id}">Record Payment</button>
                                    <button class="btn btn-xs btn-success generate-last-receipt" data-bill_id="${row.id}" data-student_id="${row.student_id}">Generate Last Receipt</button>
                                </div>
                            `;
                        } else {
                            return `
