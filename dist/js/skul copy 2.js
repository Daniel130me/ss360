...
function get_lesson_note_body() {
    ...
    // Initialize Summernote instead of CKEditor for lesson note body
                ...
                if (clipboardData && clipboardData.items) {
                    for (let i = 0; i < clipboardData.items.length; i++) {
                        ...
                        if (item.type.indexOf('image') !== -1) {
                            ...
                            if (file) uploadLessonImage(file);
                        }
                    }
                }
            }
    ...
    // Helper: upload an image file to controller and insert into summernote with progress UI
    function uploadLessonImage(file) {
        if (!file) return;
        const allowedTypes = ['image/jpeg','image/png','image/gif','image/webp','image/bmp'];
        if (!allowedTypes.includes(file.type)) {
            toastr.error('Only image files are allowed for inline uploads.');
            return;
        }
        if (file.size > 2 * 1024 * 1024) {
            toastr.error('File too large. Max 2 MB per file.');
            return;
        }
        // create or reuse progress UI
        let progressWrap = document.getElementById('lesson_image_progress_wrap');
        let progressBar = document.getElementById('lesson_image_progress');
        let statusEl = document.getElementById('lesson_image_status');
        if (!progressWrap) {
            const viewEl = document.querySelector('#lesson_note_body_view');
            if (viewEl) {
                ...
                uploadGroup.style.marginTop = '8px';
                uploadGroup.innerHTML = `
                    <div id="lesson_image_progress_wrap" style="margin-top:8px; display:block;">
                        <div style="height:6px; background:#e9ecef; border-radius:3px; overflow:hidden;">
                            <div id="lesson_image_progress" style="height:6px; width:0%; background:#007bff;"></div>
                        </div>
                        <div style="font-size:12px; margin-top:4px; color:#666; display:flex; justify-content:space-between;">
                            <span id="lesson_image_progress_text">Uploading...</span>
                            <span id="lesson_image_status" style="margin-left:10px;"></span>
                        </div>
                    </div>
                `;
                viewEl.parentNode.insertBefore(uploadGroup, viewEl.nextSibling);
                progressWrap = document.getElementById('lesson_image_progress_wrap');
                progressBar = document.getElementById('lesson_image_progress');
                statusEl = document.getElementById('lesson_image_status');
            }
        }

        const fd = new FormData();
        fd.append('file', file);
        fd.append('action', 'upload_lesson_image');
            ...
            xhr: function() {
                ...
                xhr.upload.addEventListener('progress', function(evt) {
                    if (evt.lengthComputable && progressBar) {
                        ...
                        progressBar.style.width = pct + '%';
                        const text = document.getElementById('lesson_image_progress_text');
            ...
            success: function(resp) {
                try {
                    ...
                    if (data && data.success && data.url) {
                        $('#lesson_body').summernote('insertImage', data.url);
                        if (statusEl) statusEl.textContent = 'Uploaded';
                    } else {
                        if (statusEl) statusEl.textContent = data.err || 'Upload failed';
                    }
                } catch (e) {
                    console.error('Upload response parse error', e, resp);
    ...
    $.ajax({
        ...
        success: (response) => {
            const data = !response ? '' : JSON.parse(response);
            // Set content in Summernote editor and show in the view
            const html = data.content || '';
            try {
                $('#lesson_body').summernote('code', html);
            } catch (e) {
                // fallback: set textarea value
                $('#lesson_body').val(html);
            }
            $("#lesson_body_view").html(html || "<i>No content here yet</i>");
...
function create_lesson_note_body() {
    ...
    $.ajax({
        ...
        data: {
            ...
            "body": (function(){ try { return $('#lesson_body').summernote('code'); } catch(e){ return $('#lesson_body').val(); } })(),
