...
$(document).ready(function () {
    ...
    function showPaymentReceiptModalwithbreakdown({ billId, paymentId = null, studentId = null, last = false }) {
        const $modal = $('#paymentReceiptPreviewModal');
        const $content = $('#paymentReceiptContent');
        $content.html('<div class="text-center text-muted">Loading receipt...</div>');
        $modal.modal('show');
        ...
        $.ajax({
            ...
            success: function (res) {
                if (!res.success || !res.data) {
                    $content.html('<div class="alert alert-danger text-center">Failed to load receipt.</div>');
                    return;
                }
                const d = res.data;
                // School
                ...
                // Receipt HTML
                let html = `<div class="receipt-container position-relative receipt-watermark-container p-3" id="printableReceipt">
                    <div class="">
                        <!-- Watermark logo overlay -->
                        ${//logoUrl ? `<img src="${logoUrl}" alt="Watermark Logo" class="receipt-watermark-logo">` : ''
                    ''}
                        <div class="row align-items-center mb-3 position-relative" style="z-index:2;">
    ...
    function showPaymentReceiptModal({ billId, paymentId = null, studentId = null, last = false }) {
        ...
        $modal.modal('show');
        let ajaxData = { bill_id: billId };
        // Get term_id and session_id from the page (same as DataTable)
            ...
            success: function (res) {
                ...
                let html = `<div class="receipt-container position-relative receipt-watermark-container p-3" id="printableReceipt">
                    <div class="">
                        <!-- Watermark logo overlay -->
                        ${/* logoUrl ? `<img src="${logoUrl}" alt="Watermark Logo" class="receipt-watermark-logo">` : '' */''}
                        <div class="row align-items-start mb-3 position-relative" style="z-index:2;">
                            <div class="col-2 text-center">
                                ${logoUrl ? `<img src="${logoUrl}" alt="School Logo" style="max-width:80px;max-height:80px;">` : ''}
                            </div>
                            <div class="col-10">
                                <p class="mb-0">${d.school_name || ''}</p>
                                <p class="small mb-0">${d.school_address || ''}</p>
                                <p class="small mb-0">${d.school_phone || ''}</p>
                                <p class="small mb-0">${d.school_email || ''}</p>
                            </div>
                        </div>
                        <hr>
                        
                        <p class="font-weight-bold text-center">Payment Receipt</p>
                        <div class="row mb-2">
                            <div class="col-md-6">
                                <b>Student Name:</b> ${student.firstname || ''} ${student.lastname || ''}<br>
                                <b>Admission No:</b> ${student.admission_no || ''}<br>
                            ...
                            <div class="col-md-6 text-right">
                                <b>Receipt No:</b> 000000${payment.id || ''}<br>
                                <b>Date Paid:</b> ${formatDate(payment.date_paid, true)}<br>
                                <b>Date Issued:</b> ${formatDate(d.date_issued, true)}<br>
                            </div>
                        </div>
                        <hr>
                        <div class="row">
                        ...
                        <div class="col-md-6 text-right ">
                        <div class="mb-2"><b>Term:</b> ${d.term == 1 ? "First Term" :
                        d.term == 2 ? "Second Term" :
                            d.term == 3 ? "Third Term" :
                                (d.term || '')
                    }</div>
                        <div class="mb-2"><b>Session:</b> ${d.session || ''}</div>
                        </div>
                        </div>
                        <table class="table table-sm table-bordered mb-2">
                            <tr><th>Subtotal</th><td>${money(bill.amount)}</td></tr>
                ...
                // Helper: formatDate with "25th June" style (no time)
                function formatDate(dt, shortFormatOnly = false) {
                    if (!dt) return '-';
                    const date = new Date(dt);
                    if (isNaN(date.getTime())) return dt;
    ...
    $('#quickPreviewBillButton').on('click', function () {
        // Gather Data
        ...
        $('#quickAssignInvoiceBreakdownTable tbody tr').each(function () {
            ...
            // const desc = $(this).find('input[name="quick_assign_breakdown_description[]"]').val();
            // const amt = $(this).find('input[name="quick_assign_breakdown_amount[]"]').val();
            if (desc && amt !== undefined && amt !== null && amt !== '') {
                breakdownHtml += `<tr><td>${desc}</td><td>${parseFloat(amt).toLocaleString()}</td></tr>`;
                hasBreakdown = true;
            }
        });
        ...
        if (!hasBreakdown) errors.push("Please add at least one breakdown item.");
        if (errors.length > 0) {
        ...
        let previewHtml = `
                    ...
                    <table class="table table-sm table-bordered mb-4">
                        <tr><th>Subtotal</th><td>${parseFloat(subtotal).toLocaleString()}</td></tr>
                        <tr><th>Deduction Purpose</th><td>${deductionPurpose}</td></tr>
                        <tr><th>Deduction (%)</th><td>${deductionPercentage}</td></tr>
                        <tr><th>Tax (%)</th><td>${tax}</td></tr>
                        <tr><th>Total</th><td><strong>${parseFloat(totalAmount).toLocaleString()}</strong></td></tr>
                        <tr><th>Notes</th><td>${notes}</td></tr>
                        <tr><th>Terms</th><td>${terms}</td></tr>
                    </table>
                `;
        $('#quickPreviewBillContent').html(previewHtml);
        $('#quick-preview-bill-tab-link').tab('show');
    });

    // Optionally, reset preview content on modal show
    $('#quickAssignBillModal').on('show.bs.modal', function () {
        var $quickSelect = $('#quick_assign_bill_type');
        // if ($quickSelect.data('select2')) {
        //     $quickSelect.select2('destroy');
        //     console.log("klick")
        // }
        // $quickSelect.select2({ dropdownParent: $('#quickAssignBillModal') });
        $('#quickPreviewBillContent').html('<p class="text-muted">Click the "Preview Bill" button in the Assign Bill tab to see the details here.</p>');
        $('#quick-assign-bill-tab-link').tab('show');
    });
            ...
            success: function (res) {
                if (res.success) {
                    ...
                    if (res.class_paid !== undefined) {
                        $('#total-paid-class').text('₦' + Number(res.class_paid).toLocaleString());
                    }
                    if (res.all_paid !== undefined) {
                        $('#total-paid-all').text('₦' + Number(res.all_paid).toLocaleString());
                    }
                    ...
                    render: function (data, type, row) {
                        if (row.bill_type) {
                            return `
                                ...
                                <button class="btn btn-sm btn-primary view-bill" data-id="${row.id}">View Payment Record</button>
                                <button class="btn btn-sm btn-primary edit-bill" data-id="${row.id}" data-student_id="${row.student_id}">Update Assigned Bill</button>
                                    <button class="btn btn-sm btn-primary record-payment" data-id="${row.id}" data-student_id="${row.student_id}">Record Payment</button>
                                    <button class="btn btn-sm btn-success generate-last-receipt" data-bill_id="${row.id}" data-student_id="${row.student_id}">Generate Last Receipt</button>
                                </div>
                            `;
                        } else {
                            return `
                                <div class="btn-group" role="group" aria-label="Actions">
                                    <button class="btn btn-sm btn-success quick-assign-bill" 
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $('#editTaxPercentage').val(response.data.tax);
    ...
    $previewButton.on('click', function () {
        ...
        // --- 3. Format Data for Preview ---
        let previewHtml = `
                    <h4>Bill Details</h4>
                    <table class="table table-sm table-bordered mb-4">
                        <tbody>
                            <tr><th>Bill Type:</th><td>${escapeHtml(billTypeName)}</td></tr>
                            <tr><th>Term:</th><td>${escapeHtml(termName)}</td></tr>
                            <tr><th>Session:</th><td>${escapeHtml(sessionName)}</td></tr>
                            <tr><th>Students Selected:</th><td>${studentCount}</td></tr>
                        </tbody>
                    </table>
                    <h4>Bill Breakdown</h4>
                    <table class="table table-bordered mb-4">
                        ...
                        <tbody>
                            ${(() => {
                let rows = '';
                $('#assignInvoiceBreakdownTableAssign tbody tr').each(function () { // Corrected table ID
                    const desc = $(this).find('input').eq(0).val() || '';
                    const amt = $(this).find('input').eq(1).val() || '';
                    ...
                    if (desc || amt) {
                        rows += `<tr><td>${escapeHtml(desc)}</td><td>${escapeHtml(amt)}</td></tr>`;
                    }
                });
                return rows || '<tr><td colspan="2" class="text-muted">No items</td></tr>';
            })()}
                        </tbody>
                    </table>
                    <table class="table table-sm table-bordered mb-4">
                        <tbody>
                            <tr><th>Subtotal:</th><td>${escapeHtml($('#assignSubtotal').val() || '')}</td></tr>
                            <tr><th>Deduction Purpose:</th><td>${escapeHtml($('#assignDeductionPurpose').val() || '')}</td></tr>
                            <tr><th>Deduction Percentage:</th><td>${escapeHtml($('#assignDeductionPercentage').val() || '')}</td></tr>
                            <tr><th>Tax (%):</th><td>${escapeHtml($('#assignTaxPercentage').val() || '')}</td></tr>
                            <tr><th>Total Amount:</th><td>${escapeHtml($('#assignTotalAmount').val() || '')}</td></tr>
                            <tr><th>Notes:</th><td>${escapeHtml($('#assignNotes').val() || '')}</td></tr>
                            <tr><th>Terms:</th><td>${escapeHtml($('#assignTerms').val() || '')}</td></tr>
                        </tbody>
                    </table>
        ...
        for (const studentId in selectedStudents) {
            ...
            const className = student.class_name || 'Unknown Class'; // Fallback
        ...
        if (sortedClassNames.length > 0) {
            sortedClassNames.forEach(className => {
                previewHtml += `<h5 class="mt-3">${escapeHtml(className)}</h5>`;
                previewHtml += `<table class="table table-sm table-striped table-bordered">
                                            <thead>
                                                <tr><th>#</th><th>Student Name</th><th>Admission No</th></tr>
                                            </thead>
                                            <tbody>`;
                // Sort students within the class alphabetically by name
            ...
            previewHtml += '<p class="text-muted">No students selected.</p>'; // Should not happen due to validation, but good fallback
        }


        // --- 4. Inject HTML and Switch Tab ---
        $previewContent.html(previewHtml);
        $previewTabLink.tab('show'); // Activate the Preview Bill tab
    });
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    if (typeof table !== 'undefined' && table.ajax) {
                        ...
                        // Fallback: Reload the page if table instance isn't available
                        console.warn('DataTable instance not found for reloading. Consider making it globally accessible or re-initializing.');
                        $('#student_payment_recordTable').DataTable().ajax.reload(); // Try re-fetching the instance
                    }
                } else {
    ...
    $('#editPreviewBillButton').on('click', function () {
        // Gather Data
    ...
    $('#quick_assign_bill_type').on('change', function () {
        ...
        console.log(selectedTypeId); // For debugging
        console.log(loadedBillTypesData)
    ...
    function populateBillTypeDetails(billTypeId, mode) {
        if (billTypeId && loadedBillTypesData[billTypeId]) {
            ...
            if (mode === 'quick') {
                ...
                $('#quick_assign_amount').val(billData.amount);
