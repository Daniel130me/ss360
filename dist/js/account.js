...
$(document).ready(function () {
    ...
    // Unified handler for opening the receipt preview modal.
    // Listen for any element that exposes `data-bill_id` and targets the receipt modal,
    // not just <a> tags. This lets buttons or other controls open the modal.
    $(document).on('click', '[data-bill_id][aria-controls="paymentReceiptPreviewModal"]', function (e) {
        // If this is an anchor link, prevent default navigation
        if ($(this).is('a')) e.preventDefault();

        const billId = $(this).data('bill_id');
        const paymentId = $(this).data('paymentid') || null;
        const studentId = $(this).data('student_id') || null;
        // Allow explicit data-last="1" or data-last="true" to request the last receipt
        const lastAttr = $(this).data('last');
        const last = (typeof lastAttr !== 'undefined' && (lastAttr === true || String(lastAttr) === '1' || String(lastAttr).toLowerCase() === 'true')) || $(this).hasClass('generate-last-receipt');

        if (!billId) return; // nothing to do

        showPaymentReceiptModal({ billId, paymentId, studentId, last });
    });
