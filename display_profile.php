...
$select = mysqli_query($conn, "SELECT * FROM staff WHERE id='{$_SESSION['userid']}' AND school_id='$school_id'");
...
?>
    ...
    <div class="mt-4">
        <p class="font-weight-bold small muted-text">Namom</p>
    ...
    <div class="mt-3">
        <p class="font-weight-bold small muted-text">Security PIN</p>
        <p><?= $_SESSION['userid'].$_SESSION['school_id']  ?></p>
    </div>
<?php 
}
...
<div class="row mt-3">
    <div class="col-12 col-md-auto mb-2 mb-md-0">
        <button class="btn btn-primary btn-block btn-md-auto" onclick="edit_staff_info('<?= $row['id'] ?>','<?= $row['staff_type'] ?>')">Edit Profile</button>
    </div>
    <div>
    </div>
</div>
